...
import (
	...
	"errors"
	"fmt"
	"github.com/ExploitSuite/req/v3/internal/common"
...
func (t *Transport) PutIdleTestConnH2(scheme, addr string, alt http.RoundTripper) bool {
	key := connectMethodKey{"", scheme, addr, false}
...
func makeTestDial(t *testing.T) (*testConnSet, func(ctx context.Context, n, addr string) (net.Conn, error)) {
	connSet := &testConnSet{
		t:      t,
...
func TestTransportRespectRequestWantsClose(t *testing.T) {
	...
	for _, testCase := range tests {
		t.Run(fmt.Sprintf("DisableKeepAlive=%v,RequestClose=%v", testCase.disableKeepAlives, testCase.close),
			func(t *testing.T) {
				defer afterTest(t)
				ts := httptest.NewServer(hostPortHandler)
				defer ts.Close()
				...
				c := tc().httpClient
				c.Transport.(*Transport).DisableKeepAlives = testCase.disableKeepAlives
				...
				req.Close = testCase.close
				res, err := c.Do(req)
				if err != nil {
					t.Fatal(err)
				}
				defer res.Body.Close()
				if want := testCase.disableKeepAlives || testCase.close; count > 1 || (count == 1) != want {
...
func TestTransportRemovesDeadIdleConnections(t *testing.T) {
	...
	var keys2 []string
	if !tests.WaitCondition(3*time.Second, 50*time.Millisecond, func() bool {
...
// Test that the modification made to the Request by the http.RoundTripper is cleaned up
func TestRoundTripGzip(t *testing.T) {
	...
	for i, test := range roundTripTests {
		...
		req, _ := http.NewRequest("GET", fmt.Sprintf("%s/?testnum=%d&expect_accept=%s", ts.URL, i, test.expectAccept), nil)
		if test.accept != "" {
...
func TestTransportExpect100Continue(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(rw http.ResponseWriter, req *http.Request) {
		...
		case "/keepalive":
			...
			_, bufrw, err := rw.(http.Hijacker).Hijack()
			if err != nil {
				log.Fatal(err)
			}
	...
	for i, v := range tests {
		...
		req, err := http.NewRequest("PUT", ts.URL+v.path, body)
		if err != nil {
			t.Fatal(err)
		}
...
func TestSOCKS5Proxy(t *testing.T) {
	...
	for _, useTLS := range []bool{false, true} {
		t.Run(fmt.Sprintf("useTLS=%v", useTLS), func(t *testing.T) {
			...
			go proxy(t)
			c := tc().httpClient
			c.Transport.(*Transport).Proxy = http.ProxyURL(pu)
			r, err := c.Head(ts.URL)
			if err != nil {
				t.Fatal(err)
			}
...
func TestTransportDialPreservesNetOpProxyError(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "http://fake.tld", nil)
	res, err := c.Do(req)
	if err == nil {
		res.Body.Close()
...
func TestTransportProxyDialDoesNotMutateProxyConnectHeader(t *testing.T) {
	...
	proxy := httptest.NewTLSServer(http.NotFoundHandler())
	defer proxy.Close()
	c := tc().httpClient
	...
	req, err := http.NewRequest("GET", "https://golang.fake.tld/", nil)
	if err != nil {
		t.Fatal(err)
	}
	_, err = c.Do(req)
...
func TestIssue3595(t *testing.T) {
	...
	defer ts.Close()
	c := tc().httpClient
	res, err := c.Post(ts.URL, "application/octet-stream", tests.NeverEnding('a'))
	if err != nil {
		t.Errorf("Post: %v", err)
		return
	}
...
func TestIssue4191_InfiniteGetTimeout(t *testing.T) {
	...
	c := tc().httpClient
	c.Transport.(*Transport).DialContext = func(_ context.Context, n, addr string) (net.Conn, error) {
...
func TestIssue4191_InfiniteGetToPutTimeout(t *testing.T) {
	...
	for i := 0; i < nRuns; i++ {
		...
		req, _ := http.NewRequest("PUT", ts.URL+"/put", sres.Body)
		_, err = c.Do(req)
		if err == nil {
			sres.Body.Close()
...
func reqWithT(r *http.Request, t *testing.T) *http.Request {
...
func testTransportCancelRequestInDo(t *testing.T, body io.Reader) {
	...
	req, _ := http.NewRequest("GET", ts.URL, body)
	go func() {
		defer close(donec)
		c.Do(req)
	}()
...
%s`, resCode, id, len(body), body), "\n", "\r\n", -1))
	...
	testResponse := func(req *http.Request, name string, wantCode int) {
		t.Helper()
		res, err := c.Do(req)
		if err != nil {
	...
	for i := 1; i <= numReqs; i++ {
		req, _ := http.NewRequest("POST", "http://dummy.tld/", strings.NewReader(reqBody(i)))
...
func newClientServerTest(t *testing.T, h2 bool, h http.Handler, opts ...interface{}) *clientServerTest {
	...
	nethttp2.ConfigureServer(cst.ts.Config, nil)
...
func TestTransportIgnore1xxResponses(t *testing.T) {
	setParallel(t)
	defer afterTest(t)
	cst := newClientServerTest(t, h1Mode, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		conn, buf, _ := w.(http.Hijacker).Hijack()
	...
	req, _ := http.NewRequest("GET", cst.ts.URL, nil)
...
func testProxyForRequest(t *testing.T, tt proxyFromEnvTest, proxyForRequest func(req *http.Request) (*url.URL, error)) {
	t.Helper()
	reqURL := tt.req
	if reqURL == "" {
...
func TestProxyFromEnvironment(t *testing.T) {
	...
	for _, tt := range proxyFromEnvTests {
		testProxyForRequest(t, tt, func(req *http.Request) (*url.URL, error) {
...
func TestIdleConnChannelLeak(t *testing.T) {
	...
	c := tc().httpClient
	tr := c.Transport.(*Transport)
	tr.DialContext = func(_ context.Context, netw, addr string) (net.Conn, error) {
...
func TestTransportNoReuseAfterEarlyResponse(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", ts.URL, io.MultiReader(io.LimitReader(tests.NeverEnding('x'), bodySize-1), finalBit))
...
func TestTransportClosesBodyOnError(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", ts.URL, struct {
		io.Reader
		io.Closer
	}{
		io.MultiReader(io.LimitReader(tests.NeverEnding('x'), 1<<20), iotest.ErrReader(fakeErr)),
...
func TestTransportDialTLS(t *testing.T) {
	...
	defer ts.Close()
	c := tc().httpClient
	c.Transport.(*Transport).DialTLSContext = func(_ context.Context, netw, addr string) (net.Conn, error) {
		mu.Lock()
		didDial = true
		mu.Unlock()
...
func TestTransportRemovesH2ConnsAfterIdle(t *testing.T) {
	...
	cst := newClientServerTest(t, h2Mode, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}), trFunc)
	defer cst.close()
...
func TestTransportFlushesBodyChunks(t *testing.T) {
	...
	go func() {
		req, _ := http.NewRequest("POST", "http://localhost:8080", bodyr)
	...
	req, err := http.ReadRequest(bufio.NewReader(connr))
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransportFlushesRequestHeader(t *testing.T) {
	...
	pr, pw := io.Pipe()
	req, err := http.NewRequest("POST", cst.ts.URL, pr)
	if err != nil {
		t.Fatal(err)
	}
...
func TestTransportPrefersResponseOverWriteError(t *testing.T) {
	...
	for i := 0; i < count; i++ {
		req, err := http.NewRequest("PUT", ts.URL, strings.NewReader(bigBody))
		if err != nil {
			t.Fatal(err)
		}
		resp, err := c.Do(req)
...
func testTransportEventTrace(t *testing.T, h2 bool, noHooks bool) {
	...
	cst := newClientServerTest(t, h2, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "GET" {
	...
	ctx := context.WithValue(context.Background(), lookupIPAltResolverKey{}, func(ctx context.Context, network, host string) ([]net.IPAddr, error) {
	...
	body := "some body"
	req, _ := http.NewRequest("POST", cst.scheme()+"://dns-is-faked.golang:"+port, strings.NewReader(body))
	...
	req, _ = http.NewRequest("GET", cst.scheme()+"://dns-is-faked.golang:"+port, nil)
	req = req.WithContext(httptrace.WithClientTrace(ctx, trace))
	res, err = cst.c.Do(req)
	if err != nil {
		t.Fatal(err)
...
func TestTransportEventTraceTLSVerify(t *testing.T) {
	...
	wantOnce("x509: certificate is valid for example.com")
...
func TestTransportEventTraceRealDNS(t *testing.T) {
	...
	req, _ := http.NewRequest("GET", "http://dns-should-not-resolve.golang:80", nil)
	trace := &httptrace.ClientTrace{
...
func TestTransportRejectsAlphaPort(t *testing.T) {
	res, err := http.Get("http://dummy.tld:123foo/bar")
	if err == nil {
		res.Body.Close()
...
func TestIdleConnH2Crash(t *testing.T) {
	setParallel(t)
	cst := newClientServerTest(t, h2Mode, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// nothing
	}))
	defer cst.close()
	...
	cst.tr.DialTLSContext = func(_ context.Context, network, addr string) (net.Conn, error) {
...
func TestTransportProxyConnectHeader(t *testing.T) {
	...
	c := tc().httpClient
	c.Transport.(*Transport).Proxy = func(r *http.Request) (*url.URL, error) {
		return url.Parse(ts.URL)
	}
	c.Transport.(*Transport).ProxyConnectHeader = http.Header{
		"User-Agent": {"foo"},
		"Other":      {"bar"},
	}
...
func TestTransportProxyGetConnectHeader(t *testing.T) {
	...
	c.Transport.(*Transport).GetProxyConnectHeader = func(ctx context.Context, proxyURL *url.URL, target string) (http.Header, error) {
		return http.Header{
			"User-Agent": {"foo2"},
			"Other":      {"bar2"},
		}, nil
...
func (fn funcRoundTripper) RoundTrip(*http.Request) (*http.Response, error) {
	fn()
...
func doFetchCheckPanic(tr *Transport, req *http.Request) (res *http.Response, err error, panicked bool) {
	defer func() {
		if r := recover(); r != nil {
			panicked = true
		}
...
func TestTransportRequestReplayable(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			req:  &http.Request{Method: "GET", Body: NoBody},
			want: true,
		},
		{
			name: "GET_body",
			req:  &http.Request{Method: "GET", Body: someBody},
			want: false,
		},
		{
			name: "POST",
		...
		},
		{
			...
			req:  &http.Request{Method: "POST", Header: http.Header{"Idempotency-Key": {"x"}}},
			want: true,
		},
		{
			name: "POST_x-idempotency-key",
			req:  &http.Request{Method: "POST", Header: http.Header{"X-Idempotency-Key": {"x"}}},
			want: true,
		},
		{
			name: "POST_body",
			req:  &http.Request{Method: "POST", Header: http.Header{"Idempotency-Key": {"x"}}, Body: someBody},
			want: false,
		},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := isReplayable(tt.req)
			if got != tt.want {
...
func TestTransportRequestWriteRoundTrip(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			req, err := http.NewRequest("PUT", cst.ts.URL, r)
			if err != nil {
				t.Fatal(err)
			}
...
func TestIs408(t *testing.T) {
	...
	for _, tt := range tests {
		if got := is408Message([]byte(tt.in)); got != tt.want {
...
func TestTransportIgnores408(t *testing.T) {
	...
	defer cst.close()
	req, err := http.NewRequest("GET", cst.ts.URL, nil)
	if err != nil {
		t.Fatal(err)
	}
	res, err := cst.c.Do(req)
...
func TestTransportClosesBodyOnInvalidRequests(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			req := tt.req
			req.Body = &bc
			_, err := DefaultClient().httpClient.Do(tt.req)
			if err == nil {
...
func TestDontCacheBrokenHTTP2Conn(t *testing.T) {
	cst := newClientServerTest(t, h2Mode, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {}), optQuietLog)
	defer cst.close()
	...
	cst.tr.DialContext = func(_ context.Context, netw, addr string) (net.Conn, error) {
	...
	for i := 1; i <= numReqs; i++ {
		...
		req, err := http.NewRequestWithContext(ctx, "GET", cst.ts.URL, nil)
		if err != nil {
			t.Fatal(err)
		}
		_, err = cst.c.Do(req)
...
func testTransportRace(req *http.Request) {
	save := req.Body
	pr, pw := io.Pipe()
	defer pr.Close()
	defer pw.Close()
...
func TestCancelRequestWhenSharingConnection(t *testing.T) {
	...
	go func() {
		defer wg.Done()
		req, _ := http.NewRequestWithContext(cancelctx, "GET", ts.URL, nil)
		res, err := client.Do(req)
		if err == nil {
			res.Body.Close()
		}
